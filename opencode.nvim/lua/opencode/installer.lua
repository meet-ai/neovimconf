...
local install_cmds = {
  ...
  pnpm = 'pnpm add -g @anomalyco/opencode',
  yarn = 'yarn global add @anomalyco/opencode',
  bun = 'bun add -g @anomalyco/opencode',
  deno = [[deno install --global --allow-all -f --name opencode npm:@anomalyco/opencode]],
}

local fallback_instructions = {
  pnpm = [[
    pnpm installed successfully, but Opencode may not be in your PATH.
    Run `pnpm setup`, then restart your shell or add the output of `pnpm bin -g` to your PATH.
  ]],
  yarn = [[
    Yarn installed Opencode, but the CLI may not be in your PATH.
  ...
  bun = [[
    Bun installed Opencode, but it may not be available in your PATH.
  ...
  deno = [[
    Deno installed Opencode, but it may not be in your PATH.
...
function M.open_install_float()
  ...
  vim.api.nvim_buf_set_option(state.buf, 'filetype', 'opencode')
...
function M.run_install(pm, on_success)
  ...
  if not cmd_str then
    vim.notify('[opencode.nvim] Unsupported package manager: ' .. pm, vim.log.levels.ERROR)
    return
  end
  ...
  state.job = vim.fn.termopen(cmd_str, {
    ...
    on_exit = function(_, code)
      if code == 0 then
        vim.notify('[opencode.nvim] opencode CLI installed successfully via ' .. pm, vim.log.levels.INFO)
        if not M.__test_ignore_path_check and vim.fn.executable 'opencode' == 0 then
          local fallback = fallback_instructions[pm]
          if fallback then
            vim.notify('[opencode.nvim] CLI not yet available on PATH.\n' .. fallback, vim.log.levels.WARN)
          else
            vim.notify('[opencode.nvim] CLI not found in PATH after install. Try restarting your shell.', vim.log.levels.WARN)
          end
        end
        if on_success then
        ...
        if not M.__test_ignore_path_check then
          vim.notify('[opencode.nvim] Installation failed via ' .. pm, vim.log.levels.ERROR)
...
function M.prompt_autoinstall(on_done)
  ...
  vim.schedule(function()
    vim.ui.select(pms, {
      prompt = 'Select package manager to install @anomalyco/opencode:',
      kind = 'opencode-install',
    }, function(choice)
      if not choice then
        on_done(false)
        return
