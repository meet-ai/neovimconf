...
if list_only_embed then
    ...
    local llm_name = json_txt(p.llm_feature_name, "")
    local h_label = json_txt(p.heuristic_label, "")
    local g_label = json_txt(p.gitnexus_label, "")
    local desc = json_txt(p.llm_feature_description, "")
    local core = json_txt(p.llm_core_logic_summary, "")
    local uses = json_txt(p.llm_use_cases, "")
    local header_info = ""
    if llm_name ~= "" then
